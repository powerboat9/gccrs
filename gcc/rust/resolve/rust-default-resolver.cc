...
void
DefaultResolver::visit_extern_crate (AST::ExternCrate &extern_crate, AST::Crate &crate, CrateNum num)
{
  visit (crate);
}
...
DefaultResolver::visit (AST::ExternCrate &crate)
{
  ...
  auto sub_visitor_1 = [&, this] () {
    visit_extern_crate (crate, referenced_crate, num);
  };
  ...
  if (crate.has_as_clause ())
    ctx.scoped (Rib::Kind::Module, referenced_crate.get_node_id (), sub_visitor_2,
		crate.get_as_clause ());
  else
    ctx.scoped (Rib::Kind::Module, referenced_crate.get_node_id (), sub_visitor_2,
		crate.get_referenced_crate ());
